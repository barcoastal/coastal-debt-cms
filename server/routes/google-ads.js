...
// Uses campaign-level average CPC per date (more reliable than per-GCLID click_view)
async function fetchMissingCosts() {
  ...
  try {
    ...
    // Query campaign-level cost and clicks grouped by date (search campaigns only)
    const query = `
      SELECT
        metrics.cost_micros,
        metrics.clicks,
